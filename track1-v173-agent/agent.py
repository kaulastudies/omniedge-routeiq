...
def batch_response_format() -> dict[str, Any]:
    # JSON mode is materially smaller than transmitting a full schema.
    # The parser and validators still enforce RouteIQ's output contract.
...
def request_batch(
    ...
    system_prompt = (
        'Solve q accurately. Return JSON only as {"r":[[i,"a"],...]}. '
        'Include every index once. Obey each task format, length, and language. '
        'a is the final answer; include explanation or complete code only when requested.'
    )

    compact = json.loads(compact_tasks(tasks))
    user_content: Any = compact
    if repair_reasons:
        system_prompt += ' Fix every supplied validation issue.'
        user_content = {
            "q": compact,
            "x": [
                [index, repair_reasons.get(str(task["task_id"]), "invalid")]
                for index, task in enumerate(tasks)
            ],
        }
