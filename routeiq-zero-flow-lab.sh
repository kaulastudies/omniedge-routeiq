...
#
# Optional comparison with the untouched original Qwen flow:
...
AGENT_DIR="${AGENT_DIR:-track1-v181-agent}"
...
if [[ ! -d "$AGENT_DIR" ]]; then
  if [[ -f "agent.py" && -d "local" ]]; then
    AGENT_DIR="."
  else
    fail "Could not find '$AGENT_DIR'. Set AGENT_DIR to the folder containing agent.py and local/."
  fi
fi
