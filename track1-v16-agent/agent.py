...
def remote_token_budget(tasks: list[dict[str, Any]]) -> int:
    per_task = {
        "factual": 192,
        "math": 160,
        "sentiment": 160,
    ...
    total = sum(per_task.get(infer_category(task), 220) for task in tasks)
    return min(4096, max(96, total))
        ...
        "messages": [
            {
                "role": "system",
                "content": (
                    'Solve accurately. Obey exact requested format. Return JSON '
                    '{"r":[{"i":"task id","a":"answer"}]}; include every task once. '
                    "No reasoning unless requested; code must be complete."
                ),
            },
            {"role": "user", "content": compact_tasks(tasks)},
        ],
        "temperature": 0,
        "max_tokens": remote_token_budget(tasks),
        "stream": False,
...
def resolve_local_tasks(tasks: list[dict[str, Any]]) -> tuple[dict[str, str], list[dict[str, Any]]]:
    ...
    for task in tasks:
        ...
        decisions = (
            ...
            ("logic", solve_logic(text)),
            ("ner", solve_ner(text)),
            ("code", solve_code(task)),
            ("summary", solve_summary(task)),
        )
        accepted = False
        reasons: list[str] = []
...
def requires_verified_geography(task: dict[str, Any]) -> bool:
    if infer_category(task) != "factual":
        return False
    prompt = task_prompt(task).lower()
    explicit = any(term in prompt for term in (
        "body of water", "what water", "which water", "nearby water",
        "river is it near", "lake is it near", "sea is it near",
        "ocean is it near", "located near which river", "located near which lake",
    ))
    inferred = (
        any(term in prompt for term in (" near", "near ", "beside", "adjacent", "closest", "nearby"))
        and any(term in prompt for term in ("capital", "city", "town", "country", "location", "located"))
        and any(term in prompt for term in ("water", "river", "lake", "sea", "ocean", "bay", "harbour", "harbor"))
    )
...
def _validated_subset(tasks: list[dict[str, Any]], proposed: dict[str, str], source: str) -> dict[str, str]:
    ...
    for task_id, answer in proposed.items():
        ...
        if not task:
            continue
        if source == "Qwen" and requires_verified_geography(task):
            log(f"Qwen validation rejected {task_id}: geographic proximity requires verified fallback")
            continue
...
def main() -> int:
    ...
    if unresolved:
        try:
            log(f"Attempting one local Qwen batch for {len(unresolved)} tasks")
            raw_qwen = call_qwen_batch(unresolved)
            qwen_answers = _validated_subset(unresolved, raw_qwen, "Qwen")
            answers.update(qwen_answers)
            log(f"Qwen accepted {len(qwen_answers)} of {len(raw_qwen)} recovered answers")
        except Exception as error:
            log(f"Local Qwen batch failed: {type(error).__name__}: {error}")
    ...
    log(
        f"Routing summary: deterministic={len(local_answers)}; "
        f"qwen={len(qwen_answers)}; remote={len(remote_candidates)}"
    )

    local_only = os.getenv("ROUTEIQ_LOCAL_ONLY", "0").strip() == "1"
    if remote_candidates and not local_only:
        ...
        if models and url and api_key and seconds_left() >= 25:
            ...
            missing = [
                task for task in remote_candidates
                if not answers.get(str(task["task_id"]), "").strip()
            ]
            if missing:
    ...
    log(
        f"Wrote {len(results)} results; completed={completed}; missing={len(results)-completed}; "
        f"deterministic={len(local_answers)}; qwen={len(qwen_answers)}; "
        f"remote_needed={len(remote_candidates)}"
    )
    return 0
